...
Const DEFAULT_HOST = "0.0.0.0" ' for local
...
Do
    ...
    If connections Then
        For c = 1 To MAX_CLIENTS
            ...
            If client_handle(c) Then
                ...
                Rem                 Print "Processing connection #" + Str$(c) + " (" + Str$(ROUND((Timer(.001) - client_expiry(c)) / 1000, 1)) + "ms old)"
    ...
    _Limit 1000 ' default 100, range 1-1000. Higher numbers = better perf at cost of higher idle CPU usage
...
Sub respond_page (c As Integer, header As String, pagename As String, content_type As String)
    ...
    Do While Not EOF(1)
        ...
        If InStr(line$, "<!--$BODY-->") Then
            ...
            page_file$ = "./web/pages/" + pagename + ".html"
            If Not _FileExists(page_file$) Then
                Print "ERROR: page file not found: " + page_file$
                send c, "<h1>404 - Page not found</h1>"
                send c, "<p>Missing file: " + page_file$ + "</p>"
            Else
                Open page_file$ For Input As #2
                Do While Not EOF(2)
...
Sub read_metadata (pagename As String, title As String)
    ...
    page_file$ = "./web/pages/" + pagename + ".html"
    If Not _FileExists(page_file$) Then
        Print "ERROR: metadata page file not found: " + page_file$
        title = pagename
        Exit Sub
    End If

    Open page_file$ For Input As #3
